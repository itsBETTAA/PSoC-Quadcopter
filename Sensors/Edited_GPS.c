...
volatile uint8_t recvdflag; //should uint8_t remain uint8_t?
volatile uint8_t inStandbyMode;

// CHANGE MADE HERE
uint8_t parse(char *nmea) {
  // do checksum check
   ...
   // found RMC
    ...
    if (',' != *p)
    {
      ...
      if (p[0] == 'W') lon = 'W';
        ...
        return false;} // Formatting adjustment
    }
    // speed
    p = strchr(p, ',')+1;
    if (',' != *p)
...
char read(void) {
  char c = 0;
 if (gpsSerial_SpiUartGetRxBufferSize()< 1)
  return c;

 c= gpsSerial_UartGetByte();
