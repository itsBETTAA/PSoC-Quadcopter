...
uint8_t baro_begin()
{
    ...
    ms5611_osr_t osr = MS5611_LOW_POWER;
    Wire_begin();
    baro_reset();
    baro_setOversampling(osr);
    CyDelay(100);
...
double baro_getAbsoluteAltitude(int32_t pressure)
{
    ...
    double ret = (44330.0f * (1.0f - pow(((double)pressure / (double)seaLevelPressure), 0.19029490f)));
    return ret;
}
...
{
    double ret = (44330.0f * (1.0f - pow(((double)pressure / (double)initialPressure), 0.1902949f)));
    return ret;
}
...
{
    double ret = ((double)pressure / pow((1.0) - ((double)altitude / (44330.0)), (5.255)));
    return ret;
}
