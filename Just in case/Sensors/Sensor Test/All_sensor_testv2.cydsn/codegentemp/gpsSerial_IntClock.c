/*******************************************************************************
* File Name: gpsSerial_IntClock.c
* Version 2.20
*
*  Description:
...
/*******************************************************************************
* Function Name: gpsSerial_IntClock_Start
********************************************************************************
*
* Summary:
...
void gpsSerial_IntClock_Start(void) 
{
    ...
    gpsSerial_IntClock_CLKEN |= gpsSerial_IntClock_CLKEN_MASK;
	gpsSerial_IntClock_CLKSTBY |= gpsSerial_IntClock_CLKSTBY_MASK;
}


/*******************************************************************************
* Function Name: gpsSerial_IntClock_Stop
********************************************************************************
*
* Summary:
...
void gpsSerial_IntClock_Stop(void) 
{
    ...
    gpsSerial_IntClock_CLKEN &= (uint8)(~gpsSerial_IntClock_CLKEN_MASK);
	gpsSerial_IntClock_CLKSTBY &= (uint8)(~gpsSerial_IntClock_CLKSTBY_MASK);
}
...
/*******************************************************************************
* Function Name: gpsSerial_IntClock_StopBlock
********************************************************************************
*
* Summary:
...
{
    if ((gpsSerial_IntClock_CLKEN & gpsSerial_IntClock_CLKEN_MASK) != 0u)
    {
        ...
        CLK_DIST_AMASK = gpsSerial_IntClock_CLKEN_MASK;
        CLK_DIST_DMASK = 0x00u;
#else
        CLK_DIST_DMASK = gpsSerial_IntClock_CLKEN_MASK;
        CLK_DIST_AMASK = 0x00u;
#endif /* gpsSerial_IntClock__CFG3 */

        /* Clear mask of bus clock. */
        CLK_DIST_BCFG2 &= (uint8)(~BCFG2_MASK);

        oldDivider = CY_GET_REG16(gpsSerial_IntClock_DIV_PTR);
        ...
        CLK_DIST_LD = 0x00u;
        CY_SET_REG16(gpsSerial_IntClock_DIV_PTR, oldDivider);
#endif /* HAS_CLKDIST_LD_DISABLE */
    }
}
...
/*******************************************************************************
* Function Name: gpsSerial_IntClock_StandbyPower
********************************************************************************
*
* Summary:
...
/*******************************************************************************
* Function Name: gpsSerial_IntClock_SetDividerRegister
********************************************************************************
*
* Summary:
                                ...
                                
{
    ...
    uint8 currSrc = gpsSerial_IntClock_GetSourceRegister();
    uint16 oldDivider = gpsSerial_IntClock_GetDividerRegister();

    if (clkDivider != oldDivider)
    {
        enabled = gpsSerial_IntClock_CLKEN & gpsSerial_IntClock_CLKEN_MASK;
        ...
        if ((currSrc == (uint8)CYCLK_SRC_SEL_CLK_SYNC_D) && ((oldDivider == 0u) || (clkDivider == 0u)))
        {
            ...
            if (oldDivider == 0u)
            {
                ...
                CY_SET_REG16(gpsSerial_IntClock_DIV_PTR, clkDivider);
                gpsSerial_IntClock_MOD_SRC &= (uint8)(~CYCLK_SSS);
            }
            else
            {
                /* Moving to SSS, set SSS which then ignores the divider and we can set     */
                /* it without bothering with the shadow load.                               */
                gpsSerial_IntClock_MOD_SRC |= CYCLK_SSS;
                CY_SET_REG16(gpsSerial_IntClock_DIV_PTR, clkDivider);
            }
        }
        else
        {
            ...
            if (enabled != 0u)
            {
                ...
                if (((gpsSerial_IntClock_MOD_SRC & CYCLK_SYNC) == 0u) || (clkDivider == 0u))
                {
#if HAS_CLKDIST_LD_DISABLE
                    CY_SET_REG16(CYREG_CLKDIST_WRK0, oldDivider);
                    CLK_DIST_LD = CYCLK_LD_DISABLE|CYCLK_LD_SYNC_EN|CYCLK_LD_LOAD;
            ...
            else
            {
                ...
                CY_SET_REG16(gpsSerial_IntClock_DIV_PTR, clkDivider);
				gpsSerial_IntClock_CLKEN |= enabled;
            }
        }
    }
}
...
/*******************************************************************************
* Function Name: gpsSerial_IntClock_GetDividerRegister
********************************************************************************
*
* Summary:
...
{
    return CY_GET_REG16(gpsSerial_IntClock_DIV_PTR);
}


/*******************************************************************************
* Function Name: gpsSerial_IntClock_SetModeRegister
********************************************************************************
*
* Summary:
...
{
    gpsSerial_IntClock_MOD_SRC |= modeBitMask & (uint8)gpsSerial_IntClock_MODE_MASK;
}


/*******************************************************************************
* Function Name: gpsSerial_IntClock_ClearModeRegister
********************************************************************************
*
* Summary:
...
{
    gpsSerial_IntClock_MOD_SRC &= (uint8)(~modeBitMask) | (uint8)(~(uint8)(gpsSerial_IntClock_MODE_MASK));
}


/*******************************************************************************
* Function Name: gpsSerial_IntClock_GetModeRegister
********************************************************************************
*
* Summary:
...
{
    return gpsSerial_IntClock_MOD_SRC & (uint8)(gpsSerial_IntClock_MODE_MASK);
}


/*******************************************************************************
* Function Name: gpsSerial_IntClock_SetSourceRegister
********************************************************************************
*
* Summary:
...
{
    uint16 currDiv = gpsSerial_IntClock_GetDividerRegister();
    uint8 oldSrc = gpsSerial_IntClock_GetSourceRegister();
        ...
        (clkSource == ((uint8)CYCLK_SRC_SEL_CLK_SYNC_D))) && (currDiv == 0u))
    {
        ...
        gpsSerial_IntClock_MOD_SRC |= CYCLK_SSS;
        gpsSerial_IntClock_MOD_SRC =
            (gpsSerial_IntClock_MOD_SRC & (uint8)(~gpsSerial_IntClock_SRC_SEL_MSK)) | clkSource;
    }
...
/*******************************************************************************
* Function Name: gpsSerial_IntClock_GetSourceRegister
********************************************************************************
*
* Summary:
...
{
    return gpsSerial_IntClock_MOD_SRC & gpsSerial_IntClock_SRC_SEL_MSK;
}
...
/*******************************************************************************
* Function Name: gpsSerial_IntClock_SetPhaseRegister
********************************************************************************
*
* Summary:
...
{
    gpsSerial_IntClock_PHASE = clkPhase & gpsSerial_IntClock_PHASE_MASK;
}


/*******************************************************************************
* Function Name: gpsSerial_IntClock_GetPhase
********************************************************************************
*
* Summary:
...
{
    return gpsSerial_IntClock_PHASE & gpsSerial_IntClock_PHASE_MASK;
}
