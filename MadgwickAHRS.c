...
static /*volatile*/ float beta; // algorithm gain
static /*volatile*/ float q0;
static /*volatile*/ float q1;
static /*volatile*/ float q2;
static /*volatile*/ float q3; // quaternion of sensor frame relative to auxiliary frame
static /*volatile*/ float invSampleFreq;
static /*volatile*/ float roll;
static /*volatile*/ float pitch;
static /*volatile*/ float yaw;
static /*volatile*/ char anglesComputed;
...
void madgwick_update(float gx, float gy, float gz, float ax, float ay, float az, float mx, float my, float mz)
{
    ...
    // but since the accel_mag library already does that
    // the following will be commented out 
    //gx *= 0.0174533f;
    //gy *= 0.0174533f;
    //gz *= 0.0174533f;
...
float getPitch(void)
{
    ...
    return (atitude.pitch * 57.29578f);
}
float getYaw(void)
{
    /*if (!anglesComputed)
    ...
    return (atitude.yaw * 57.29578f + 180.0f);
}
